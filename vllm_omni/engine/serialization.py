...
def _serialize_model_buffer_value(value: Any) -> Any:
    if isinstance(value, torch.Tensor):
        ...
        if tensor.layout != torch.strided:
            raise TypeError(
                f"model_intermediate_buffer tensor must use strided layout; received layout={tensor.layout}"
            )
        ...
        try:
            ...
            raise TypeError(
                ...
                f"dtype={tensor.dtype} shape={tuple(tensor.shape)} layout={tensor.layout}"
            ) from exc
        return {
...
def serialize_model_intermediate_buffer(
    ...
    ``OmniEngineCoreRequest.model_intermediate_buffer`` intentionally permits
    arbitrary nested metadata. Consequently vLLM's msgspec decoder sees its
    values as ``Any`` and cannot infer that the native tensor tuple emitted by
    ``MsgpackEncoder`` should be restored to ``torch.Tensor``. Encode only
    tensors into an explicit versioned envelope; ordinary containers and
    scalar values retain their existing representation.
    """
    if buffer is None:
        return None
    if not isinstance(buffer, dict):
        raise TypeError(f"model_intermediate_buffer must be a dictionary or None; received {type(buffer).__name__}")
    return _serialize_model_buffer_value(buffer)


def _decode_model_buffer_tensor(value: dict[str, Any]) -> torch.Tensor:
    ...
    if len(data) != expected_bytes:
        raise ValueError(
            ...
            f"dtype={dtype_name} shape={tuple(shape)} expected={expected_bytes} actual={len(data)}"
        )
    if expected_bytes == 0:
